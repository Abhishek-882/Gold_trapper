...
//====================================================================
// BB ALERT ARROWS (MAIN ENTRY SIGNAL)
//
// Uses the centering swing extreme algorithm from BB_Alert Arrows_.mq5:
// - Evaluates closed candle Bar 1 (confirmed signal, not Bar 0).
// - Confirmed Buy Arrow (Swing Low extreme) -> Instant Market BUY on Bar 0 open.
// - Confirmed Sell Arrow (Swing High extreme) -> Instant Market SELL on Bar 0 open.
// - 100% pure embedded calculation (zero external indicator file dependency).
//====================================================================

input group "===== BB ALERT ARROWS (MAIN ENTRY SIGNAL) =====";
input bool             EnableBBAlertSignal    = true;           // Enable BB Alert Arrows as Main Entry Signal
input ENUM_TIMEFRAMES  BBAlertTimeframe       = PERIOD_CURRENT; // BB Alert Timeframe (Current Chart Timeframe)
input int              InpBBLookbackPeriod    = 24;             // BB Alert Lookback Period (default 24)
input int              InpBBSignalGap         = 3;              // BB Alert Signal Gap in Points (default 3)
input int              InpBBSignalBar         = 1;              // Signal Confirmation Bar (1 = Confirmed Closed Candle)
...
//+------------------------------------------------------------------+
//| BB ALERT ARROWS PURE EMBEDDED CALCULATION                        |
//+------------------------------------------------------------------+
...
datetime g_lastExecutedBBSignalTime = 0;

ENUM_BB_ALERT_SIGNAL GetBBAlertSignal(int checkBar = 1) {
  if (!EnableBBAlertSignal)
    return BB_SIGNAL_NONE;

  int lookback = InpBBLookbackPeriod;
  if (lookback < 2)
    lookback = 24;

  int half = lookback / 2;
  int neededBars = checkBar + half + lookback + 10;

  double highSeries[], lowSeries[];
  ArraySetAsSeries(highSeries, true);
  ArraySetAsSeries(lowSeries, true);

  if (CopyHigh(_Symbol, BBAlertTimeframe, 0, neededBars, highSeries) < neededBars ||
      CopyLow(_Symbol, BBAlertTimeframe, 0, neededBars, lowSeries) < neededBars) {
    return BB_SIGNAL_NONE;
  }

  int start_bar = checkBar - half;
  if (start_bar < 0)
    start_bar = 0;

  int count = lookback;
  if (start_bar + count > neededBars)
    count = neededBars - start_bar;

  if (count <= 0)
    return BB_SIGNAL_NONE;

  int highest_idx = ArrayMaximum(highSeries, start_bar, count);
  int lowest_idx  = ArrayMinimum(lowSeries, start_bar, count);

  // In BB_Alert Arrows_:
  // highest_idx == checkBar -> Sell Arrow (Upper Extreme / Reversal)
  // lowest_idx  == checkBar -> Buy Arrow (Lower Extreme / Reversal)
  if (highest_idx >= 0 && checkBar == highest_idx)
    return BB_SIGNAL_SELL;

  if (lowest_idx >= 0 && checkBar == lowest_idx)
    return BB_SIGNAL_BUY;
...
void StartNewCycle() {
  ...
  bool isBBMarketEntry = false;

  if (EnableBBAlertSignal) {
    datetime signalBarTime = iTime(_Symbol, BBAlertTimeframe, InpBBSignalBar);
    if (signalBarTime <= 0)
      return;

    if (signalBarTime == g_lastExecutedBBSignalTime)
      return; // Already executed entry for this confirmed candle

    ENUM_BB_ALERT_SIGNAL bbSig = GetBBAlertSignal(InpBBSignalBar);
    if (bbSig == BB_SIGNAL_BUY) {
      prediction      = PRED_BUY;
      allowBuy        = true;
      allowSell       = false;
      isBBMarketEntry = true;
    } else if (bbSig == BB_SIGNAL_SELL) {
      prediction      = PRED_SELL;
      allowBuy        = false;
      allowSell       = true;
      isBBMarketEntry = true;
    } else {
      // No confirmed arrow on closed bar -> wait for next signal
      return;
    }
  } else if (EnableDirectionalPrediction) {
  ...
  // BUY STOP / MARKET BUY (only placed when prediction/BB/SR/trend filters allow it)
  //================================================================
  ...
  if (allowBuy) {
    ...
    if (isBBMarketEntry) {
      // Instant Market Order on Confirmed BB Alert Arrow
      double buyTP = 0;
      if (effectiveInitialTP > 0 && !willTrailBuy) {
        double actualTPDistance = MathMax(effectiveInitialTP, minimumDistance);
        buyTP = NormalizeToTickSize(ask + actualTPDistance);
        if ((buyTP - ask) < minimumDistance) {
          buyTP = NormalizeToTickSize(ask + minimumDistance + SymbolTickSize());
        }
      }

      bool buyResult = trade.Buy(lotBuy, _Symbol, ask, 0, buyTP, EAComment + "_BB_BUY");
      if (buyResult) {
        g_lastExecutedBBSignalTime = iTime(_Symbol, BBAlertTimeframe, InpBBSignalBar);
        CycleBaseLot = lotBuy;
        CycleMaxGridLevel = 1;
        Print("------------------------------------------");
        Print("BB ALERT ARROW: MARKET BUY EXECUTED");
        Print("Confirmed Bar : ", TimeToString(g_lastExecutedBBSignalTime, TIME_DATE | TIME_MINUTES));
        Print("Lot           : ", lotBuy);
        Print("Price (Ask)   : ", ask);
        Print("TP            : ", buyTP);
        Print("Trailing Mode : ", willTrailBuy ? "ON (Dynamic Runner)" : "OFF");
        Print("------------------------------------------");
      } else {
        Print("BB ALERT BUY ERROR: ", trade.ResultRetcodeDescription());
      }
    } else {
  ...
  //================================================================
  // SELL STOP / MARKET SELL (only placed when prediction/BB/SR/trend filters allow it)
  //================================================================
  ...
  if (allowSell) {
    ...
    if (isBBMarketEntry) {
      ...
      if (effectiveInitialTP > 0 && !willTrailSell) {
        double actualTPDistance = MathMax(effectiveInitialTP, minimumDistance);
        sellTP = NormalizeToTickSize(bid - actualTPDistance);
        if ((bid - sellTP) < minimumDistance) {
          sellTP = NormalizeToTickSize(bid - minimumDistance - SymbolTickSize());
        }
      }

      bool sellResult = trade.Sell(lotSell, _Symbol, bid, 0, sellTP, EAComment + "_BB_SELL");
      if (sellResult) {
        g_lastExecutedBBSignalTime = iTime(_Symbol, BBAlertTimeframe, InpBBSignalBar);
        CycleBaseLot = lotSell;
        CycleMaxGridLevel = 1;
        Print("------------------------------------------");
        Print("BB ALERT ARROW: MARKET SELL EXECUTED");
        Print("Confirmed Bar : ", TimeToString(g_lastExecutedBBSignalTime, TIME_DATE | TIME_MINUTES));
        Print("Lot           : ", lotSell);
        Print("Price (Bid)   : ", bid);
        Print("TP            : ", sellTP);
        Print("Trailing Mode : ", willTrailSell ? "ON (Dynamic Runner)" : "OFF");
        Print("------------------------------------------");
      } else {
        Print("BB ALERT SELL ERROR: ", trade.ResultRetcodeDescription());
      }
    } else {
...
void UpdateChartDisplay() {
  ...
  if (EnableDirectionalPrediction || EnableRangeBoundaryFilter || EnableSRFilter || EnableTrendFilter || EnableMacroTrendFilter) {
    ...
    if (EnableBBAlertSignal) {
      ENUM_BB_ALERT_SIGNAL curBB = GetBBAlertSignal(1);
      string bbStr = (curBB == BB_SIGNAL_BUY) ? "BUY ARROW" : ((curBB == BB_SIGNAL_SELL) ? "SELL ARROW" : "NO ARROW");
      text += "BB Alert     : " + bbStr + " (Bar 1)\n";
    }
